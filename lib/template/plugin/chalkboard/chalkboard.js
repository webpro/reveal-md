...
var RevealChalkboard = window.RevealChalkboard || (function () {
	...
	******************************************************************/
	...
	switch (theme) {
		...
		default:
			background = ['rgba(127,127,127,.1)', path + 'img/whiteboard-w.png'];
			pen = ['url(' + path + 'img/boardmarker.png), auto',
			'url(' + path + 'img/boardmarker.png), auto'];
			draw = [drawWithPen, drawOnWhiteboard];
			color = ['rgba(244,66,137,1)', 'rgba(0,0,255,1)'];
			break;
	}
	...
	******************************************************************/
	...
	if (toggleChalkboardButton) {
		...
		button.style.left = toggleChalkboardButton.left || "70px";
		...
		button.innerHTML = '<a class="btn-tools" href="#" onclick="RevealChalkboard.toggleChalkboard(); return false;"><i class="far fa-edit"></i></i></a>'
	...
	if (toggleNotesButton) {
		...
		button.style.left = toggleNotesButton.left || "110px";
		...
		button.innerHTML = '<a class="btn-tools" href="#" onclick="RevealChalkboard.toggleNotesCanvas(); return false;"><i class="fas fa-pencil-alt"></i></a>'
		document.querySelector(".reveal").appendChild(button);
	}
	//alert("Buttons");
	...
	******************************************************************/
	...
	function drawOnWhiteboard(context, fromX, fromY, toX, toY)
	{
		context.lineWidth = 3;
		context.lineCap = 'round';
	...
	******************************************************************/
	...
	this.drawOnWhiteboard = drawOnWhiteboard;
	...
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'Click the left mouse button and drag', 'Write on notes canvas or chalkboard' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'Click the right mouse button and drag', 'Wipe away previous drawings' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'DEL', 'Clear the chalkboard' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'D', 'Download chalkboard drawings' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'BACKSPACE', 'Delete all chalkboard drawings on the current slide' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'C', 'Toggle chalkboard' );
	if( window.Reveal ) Reveal.registerKeyboardShortcut( 'B', 'Toggle whiteboard' );
