...
const render = async (input, extraOptions = {}) => {
  ...
  const webManifest = options.webmanifest && getWebManifestPath(options.webmanifest, options.assetsDir, options.base);
  const serviceWorker = options.serviceworker && getServiceWorkerPath(options.serviceworker, options.base);
