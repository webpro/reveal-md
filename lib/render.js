...
function parseSlides(markdown, options) {
  ...
  const overrideYamlOptions = _.merge({}, yaml.options, options);
  const viewWithoutDefaults = parseOptions(overrideYamlOptions);
  const view = _.merge({}, viewWithoutDefaults.defaults, viewWithoutDefaults);
