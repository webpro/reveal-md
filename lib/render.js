...
function renderMarkdownFileListing(req, res) {
  ...
  const list = glob
    .sync('**/*.md', {
      ...
      ignore: '_node_modules/**'
    })
   .map( filePath => {
       let fileSplit = filePath.split( '/' );
       let fileLink = fileSplit[ fileSplit.length - 1 ];
       fileSplit.pop();
       let fileLocation = fileSplit.join( '/' );
       return `<a href="${filePath}" data-file-location="${fileLocation}">${fileLink}</a>`
   } );
