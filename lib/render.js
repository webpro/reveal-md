...
export const render = async (content, extraOptions = {}) => {
  const { yamlOptions, markdown } = parseYamlFrontMatter(content);
  ...
  const slidifyOptions = getSlidifyOptions(options);
  let slidifyAttributes = [];
  for (const [key, value] of Object.entries(slidifyOptions)) {
    if (key in slidifyAttributeNames) {
