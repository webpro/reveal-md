...
export const defaults = JSON.parse(fs.readFileSync('lib/defaults.json'));

let localConfig = undefined;
try {
  localConfig = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'reveal-md.json')));
} catch (e) {
  if (e.code !== 'ENOENT') {
    console.log(e);
  }
...
try {
  revealConfig = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'reveal.json')));
} catch (e) {
  if (e.code !== 'ENOENT') {
    console.log(e);
  }
...
const revealBasePath = path.resolve(new URL(import.meta.resolve('reveal.js')).pathname, '..', '..');
...
export const getTemplate = async template => {
  ...
  const contents = await fs.promises.readFile(path.join(base, template));
...
export const getFaviconPath = async () => {
  ...
  const hasFavicon = (await fs.promises.pathExists(faviconPath)) && isFile(faviconPath);
