...
//const revealConfig = tryRequire(path.join(__dirname, "template", 'reveal.json'));
const revealConfig = Object.assign(tryRequire(path.join(__dirname, "template", 'reveal.json')), tryRequire(path.join(process.cwd(), 'reveal.json')));
...
const getInitialDir = async () => {
  ...
  return (await isDirectory(dir)) ? dir : process.cwd()
};
