...
function getTemplate(options){
  let templateFile ='reveal.html';
  if(options.printout){
    templateFile='printout.html';
  }
...
function getBaseUrl(options){
  let base = `//${options.host}:${options.port}`;
  if(options.static||options.printout){
    base = '.';
  } else if(options.standalone){
    base = 'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/3.4.1/';
  }

  return base;
}

function getAssetPathTemplate(asset,options){
  if(options.static||options.standalone){
    return `${asset}`
  }
  return `assets/${asset}`
}
...
function parseAssetsArg(assets,options) {
  return (typeof assets === 'string' ? assets.split(',') : []).map(asset => getAssetPathTemplate(asset,options));
}
...
function parseOptions(args) {
  ...
  options.cssPaths = parseAssetsArg(args.css, options);
  ...
  options.base = () => getBaseUrl(options);
  options.template = () => getTemplate(options);
