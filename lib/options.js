...
try {
  ...
  revealOptions = require(path.join(__dirname, "template", 'reveal.json'));
} catch (err) {
  debug(err.message);
}
...
defaults.puppeteerLaunchConfig = {"pipe": true};
...
function parseOptions(options) {
  ...
  if (!options.puppeteerLaunchConfig){
    options.puppeteerLaunchConfig = {"pipe": true};
  }
