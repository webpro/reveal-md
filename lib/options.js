const fs = require('fs'),
  ...
  url = require('url'),
  _ = require('lodash'),
  internalDefaults = require('./defaults.json'),
  debug = require('debug')('reveal-md');

let defaults = internalDefaults;
try {
  const localDefaults = require(path.join(process.cwd(), 'reveal-md.json'));
  defaults = _.extend({}, internalDefaults, localDefaults);
} catch(err) {
  debug(err.message);
}
