...
module.exports = function renderStaticMarkup(options) {
  ...
  if(fs.lstatSync(options.initialPath).isDirectory()){
    ...
    const listmarkAwait = list.then(list => render.renderListFile(list,options))
      .then(html => fs.outputFileAsync(path.join(staticDir, 'index.html'), html));
    
    awaits.push(listmarkAwait);

    list.then(list => list.map(markdown => awaitPushMarkdownPage(path.join(options.initialPath, markdown),options,markdown)))
  ...
  function awaitPushMarkdownPage(file,options,subdir = null){
    ...
    const markupAwait = parseSlideAwait
      ...
      .then(markdown => {
        if(subdir != null){
          options.base=subdir.split("/").map((v)=>{return ".."}).join("/");
        }else{
          options.base = ".";
        }
        return render.render(markdown,options)
      })
      .then(markdown => fs.outputFileAsync(path.join(staticDir+(subdir != null?"/"+subdir:""), 'index.html'), markdown));
