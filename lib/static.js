...
const { getOptions, getAssetsDir, getPath } = require('./config');
...
const write = (target, content) => {
  console.log(`★ ${target}`);
  fs.outputFile(target, content);
};

const link = (source, target) => {
  console.log(`∞ ${source} → ${target}`);
  fs.ensureSymlink(source, target);
};
...
module.exports = async () => {
  ...
  const { staticDir, highlightTheme } = options;
  ...
  const awaits = ['css', 'js', 'plugin', 'lib'].map(dir =>
    cp(path.join(revealBasePath, dir), path.join(staticDir, dir))
  );
  ...
  awaits.push(writeMarkupFiles(getPath(), staticDir));

  const assets = _.compact(_.flatten([options.scripts, options.css]));
  awaits.push(...assets.map(async asset => cp(asset, path.join(staticDir, assetsDir, asset))));
