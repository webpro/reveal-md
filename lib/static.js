...
const { getOptions, getAssetsDir, getPath, getStaticDir, getSlideOptions, getFilesGlob, getPreprocessor } = require('./config');
const { isDirectory, isFile, parseYamlFrontMatter, getFilePaths } = require('./util');
...
const copyAssetsAndWriteFile = async (sourceDir, file, targetDir) => {
  extraOptions = {}
  const rawMarkdown = await fs.readFile(path.join(sourceDir, file));
  const { yamlOptions, markdown } = parseYamlFrontMatter(rawMarkdown.toString());
  ...
  const awaits = await copyAssetsFromOptions(processedMarkdown);
  let image;
  while ((image = mdImageRE.exec(processedMarkdown) || htmlImageRE.exec(processedMarkdown))) {
    const [, imgPath] = image;
